    ...
    gpointer u_data)
{
  ...
  for (l_frame = batch_meta->frame_meta_list; l_frame != NULL; l_frame = l_frame->next) {
    ...
    if ((l_obj = frame_meta->obj_meta_list)==NULL){
      g_print("NULL!!");
    }
    for (l_obj = frame_meta->obj_meta_list; l_obj != NULL; l_obj = l_obj->next) {
      ...
      int width = (int) (rect_params->width);
      int height = (int) (rect_params->height);

      int padding = 10;


      rect_params->left = left - padding;
      rect_params->top = top - padding;
      rect_params->width = width + 2 * padding;
      rect_params->height = height + 2 * padding;

      int class_index = obj_meta->class_id;
      char *text = pgie_classes_str[obj_meta->class_id];
      g_print("Frame: %d | Class %s | Left: %d Top: %d Width: %d Height: %d\n",frame_number, text, left, top, width, height);
      if (bbox_params_dump_file) {
        fprintf (bbox_params_dump_file,"Frame: %d | Class %s | Left: %d Top: %d Right: %d Bottom: %d\n",
              frame_number, text, left, top, right, bottom);
      }
...
contact: Shuo Wang (shuow@nvidia.com)");
  ...
  /* link source and video_full_processing_bin */
  if (!input_mp4){
